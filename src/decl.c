...
static Type specifier(int *sclass) {
    ...
    for (;;) {
        ...
        switch (t) {
            ...
            case SIGNED: case UNSIGNED: p = &sign; t = gettok();      break;
            case LONG: if (size == LONG) { size = 0; tt = LONG+LONG; }
            ...
            case STRUCT: case UNION:    p = &type; ty = structdcl(t); break;
            case ID: if (istypename(t, tsym) && type == 0 && sign == 0 && size == 0) {
                         use(tsym, src);
                         ty = tsym->type;
...
static void decl(Symbol (*dcl)(int, char *, Type, Coordinate *)) {
    ...
    if (t == ID || t == '*' || t == '(' || t == '[') {
        ...
        if (level == GLOBAL) {
            ...
            if (params && id && isfunc(ty1) && (t == '{' || istypename(t, tsym)
